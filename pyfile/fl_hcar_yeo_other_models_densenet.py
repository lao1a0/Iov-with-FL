...
class Arguments():
    def __init__(self):
        ...
        self.lr = 0.01
        self.num_class = 5
        self.data_train = '../data2/train_yeo_224/'
        self.data_test = '../data2/test_yeo_224/'
        # self.model_name = '../model/vgg16-397923af.pth'
        # self.save_name = 'yeo_vgg16_lr0.01_epoch50'
        # self.model_name = '../model/offical/resnet18-5c106cde.pth'
        # self.save_name = 'yeo_resnet18_lr0.01_epoch50'
        # self.model_name = '../model/offical/resnet50-11ad3fa6.pth'
        # self.save_name = 'yeo_resnet50_lr0.01_epoch50'
        self.model_name = '../model/offical/densenet121-a639ec97.pth'
        self.save_name = 'yeo_densenet121_lr0.01_epoch50'
        self.flag = False
...
def Densenet121(args):
    from torchvision.models import densenet121  # 最简单的模型
    if args.flag:
        pretrain_model = densenet121(pretrained=False)  # 导入了模型的框架
        pretrain_model.classifier = nn.Linear(pretrain_model.classifier.in_features, 5)  # 将全连接层改为自己想要的分类输出
        ...
        pretrain_model = densenet121(pretrained=False, num_classes=5).to(device)
        params_conv = pretrain_model.parameters()
    return pretrain_model, params_conv

# model, params_conv = VGG_s(args)
model, params_conv = Densenet121(args)
print('现在是在：',device)
...
file_name = '../files/{}_{}.xlsx'.format(args.save_name,datetime.now())
