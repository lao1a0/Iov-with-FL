...
def train(args, _model_, server, flag):
    ...
    # 如果文件不存在，to_csv将会创建它
    if args.name == "CNN":
        df.to_csv(args.root_save_path+"CNN/csv/{}.csv".format(model.name), mode='a', header=not os.path.exists("csv/{}.csv".format(model.name)), index=False)
    elif args.name == "ResNet":
        df.to_csv(args.root_save_path+"ResNet/csv/{}.csv".format(model.name), mode='a', header=not os.path.exists("csv/{}.csv".format(model.name)), index=False)
    elif args.name == "VGG":
        df.to_csv(args.root_save_path+"VGG/csv/{}.csv".format(model.name), mode='a', header=not os.path.exists("csv/{}.csv".format(model.name)), index=False)
    else:
        df.to_csv(args.root_save_path+"AlexNet/csv/{}.csv".format(model.name), mode='a', header=not os.path.exists("csv/{}.csv".format(model.name)), index=False)
