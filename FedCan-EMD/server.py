...
class FedProx:
    ...
    def _path(self, end):
        # 您想要检查的路径
        path = self.args.root_save_path + end
        # 检查路径是否存在
    ...
    def save_model_and_file(self, name):
        '''保存训练好的模型'''
        if name == "CNN":
            self.save_file(self._path("CNN/"))
            model_save_path = self._path("CNN/pth/")
            torch.save(self.nn.state_dict(), model_save_path + "{}.pth".format(name))
            print("保存全局模型：{} 成功".format(model_save_path + "{}.pth".format(name)))
            for i in self.nns:
                torch.save(i.state_dict(), model_save_path + "{}.pth".format(i.name))
                print("保存客户端模型：{} 成功".format(model_save_path + "{}.pth".format(i.name)))

        elif name == "ResNet":
            self.save_file(self._path("ResNet/"))
            model_save_path = self._path("ResNet/pth/")
            torch.save(self.nn_RestNet18[0].state_dict(), model_save_path + "{}.pth".format(name))
            print("保存全局模型：{} 成功".format(model_save_path + "{}.pth".format(name)))
            for i in self.nns_RestNet18:
                torch.save(i[0].state_dict(), model_save_path + "{}.pth".format(i[0].name))
                print("保存客户端模型：{} 成功".format(model_save_path + "{}.pth".format(i[0].name)))

        elif name == "VGG":
            self.save_file(self._path("VGG/"))
            model_save_path = self._path("VGG/pth/")
            torch.save(self.nn_VGG16[0].state_dict(), model_save_path + "{}.pth".format(name))
            print("保存全局模型：{} 成功".format(model_save_path + "{}.pth".format(name)))
            ...
            self.save_file(self._path("AlexNet/"))
            model_save_path = self._path("AlexNet/pth/")
            torch.save(self.nn_AlexNet[0].state_dict(), model_save_path + "{}.pth".format(name))
            print("保存全局模型：{} 成功".format(model_save_path + "{}.pth".format(name)))
