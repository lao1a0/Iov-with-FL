...
def ResNet(name):
    ...
    pretrained_dict = torch.load("/home/raoxy/FedProx-PyTorch/pth/resnet18-5c106cde.pth")
...
def VGG():
    ...
    pretrained_dict = torch.load("/home/raoxy/FedProx-PyTorch/pth/vgg16-397923af.pth")
...
def Alexnet():
    ...
    pretrain_model.classifier[6] = nn.Linear(pretrain_model.classifier[6].in_features, 5)  # 将全连接层改为自己想要的分类输出
    model_dict = pretrain_model.state_dict()
    pretrained_dict = torch.load("/home/raoxy/FedProx-PyTorch/pth/alexnet-owt-4df8aa71.pth")
    pretrained_dict.pop('classifier.6.bias')
    pretrained_dict.pop('classifier.6.weight')
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)  # 模型参数列表进行参数更新，加载参数
